...
<div class="max-w-7xl mx-auto space-y-5">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">
                All your service transactions — {{ $transactions->total() }} total
            </p>
        </div>
    </div>
    ...
    <form method="GET" action="{{ route('transactions') }}"
          class="rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm p-4">
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            {{-- Service type --}}
            <select name="service"
                    class="w-full px-3 py-2 text-sm border border-slate-200 dark:border-slate-700 rounded-xl bg-slate-50 dark:bg-slate-800 text-slate-700 dark:text-slate-300 focus:outline-none focus:ring-2 focus:ring-[color:var(--vtu-primary)]/30 focus:border-[color:var(--vtu-primary)] transition-colors">
                <option value="">All services</option>
                @foreach ($serviceTypes as $type)
                    <option value="{{ $type }}" @selected(request('service') === $type)>
                        {{ $serviceLabels[$type] ?? ucfirst($type) }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="flex items-center gap-2 mt-3">
            <button type="submit"
                    class="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold text-white rounded-xl transition-opacity hover:opacity-90"
            ...
            @if (request()->hasAny(['search', 'service', 'date_from', 'date_to']))
                <a href="{{ route('transactions') }}"
                   class="inline-flex items-center gap-1.5 px-4 py-2 text-sm font-semibold text-slate-600 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 rounded-xl hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors">
                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                        ...
                        @php
                            ...
                            $providerLabel = $networkLabels[$tx->provider] ?? ucfirst($tx->provider);
                            $serviceLabel  = $serviceLabels[$tx->service_type] ?? ucfirst($tx->service_type);
                        @endphp
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                            ...
                            <td class="px-5 py-3.5">
                                <div class="flex items-center gap-3">
                                    <div class="h-9 w-9 rounded-xl {{ $si['bg'] }} flex items-center justify-center flex-shrink-0">
                                        <svg class="h-4 w-4 {{ $si['color'] }}" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $si['icon'] }}"/>
                                        </svg>
                                    </div>
                                    <div>
                                        <p class="font-semibold text-slate-800 dark:text-slate-200 leading-tight">{{ $serviceLabel }}</p>
                                        <p class="text-xs text-slate-400 dark:text-slate-500">{{ $providerLabel }}</p>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-4 py-3.5">
                                <span class="font-mono text-sm text-slate-700 dark:text-slate-300">{{ $tx->recipient }}</span>
                            </td>

                            {{-- Reference --}}
                            <td class="px-4 py-3.5 hidden md:table-cell">
                                <span class="font-mono text-xs text-slate-400 dark:text-slate-500 select-all">{{ $tx->reference }}</span>
                            </td>
                            ...
                            <td class="px-4 py-3.5 text-center">
                                <span class="inline-flex items-center px-2.5 py-1 rounded-full text-[11px] font-semibold {{ $tx->statusBadgeClass() }}">
                                    {{ ucfirst($tx->status) }}
                                </span>
                            </td>
                            ...
                            <td class="px-5 py-3.5 text-right hidden sm:table-cell">
                                <span class="text-xs text-slate-500 dark:text-slate-400 whitespace-nowrap">
                                    ...
                                    <span class="text-slate-400 dark:text-slate-600">{{ $tx->created_at->format('h:i A') }}</span>
                                </span>
                            </td>
                    ...
                    <div class="flex items-center gap-1">
                        ...
                        @else
                            <a href="{{ $transactions->previousPageUrl() }}"
                               class="px-3 py-1.5 rounded-lg text-xs font-medium text-slate-600 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors">← Prev</a>
                        @endif
                        ...
                        @foreach ($transactions->getUrlRange(max(1, $transactions->currentPage() - 2), min($transactions->lastPage(), $transactions->currentPage() + 2)) as $page => $url)
                            @if ($page === $transactions->currentPage())
                                <span class="px-3 py-1.5 rounded-lg text-xs font-bold text-white"
                                      style="background: {{ $themeColor }}">{{ $page }}</span>
                            @else
                                <a href="{{ $url }}"
                                   class="px-3 py-1.5 rounded-lg text-xs font-medium text-slate-600 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors">{{ $page }}</a>
                            @endif
                        @endforeach
                        ...
                        @if ($transactions->hasMorePages())
                            <a href="{{ $transactions->nextPageUrl() }}"
                               class="px-3 py-1.5 rounded-lg text-xs font-medium text-slate-600 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors">Next →</a>
                        @else
